...
from visa.utils.main_utils import read_yaml_file, save_numpy_array_data, save_object, load_data
from imblearn.combine import SMOTEENN

# to do the data transformation
# we need data -> data ingestion data
# We need to validate our data
...
class DataTransformation:
    ...
    def _remove_outliers_IQR(self, col, df):
        try:
            ...
            upper_limit = percentile75 + 1.5 * iqr #upperlimit =  q3 + 1.5 * iqr ( percentile75 + 1.5 * (q3 - q1))
            lower_limit = percentile25 - 1.5 * iqr #lowerlimit =  q1 + 1.5 * iqr ( percentile75 + 1.5 * (q3 - q1))
            df.loc[(df[col]>upper_limit), col]= upper_limit  # prticular columsn ro row -> 1,2,3,4,5,6 loc[2:4] = 
    ...
    def get_data_transformer_object(self) -> ColumnTransformer:
        try:
            ...
            # missing values -> handle missing data (median)
            # missing values -> categorical data (mode)

            numerical_columns = dataset_schema[NUMERICAL_COLUMN_KEY]
            ordinal_columns = dataset_schema[ORDINAL_COLUMN_KEY]
            onehot_columns = dataset_schema[ONEHOT_COLUMN_KEY]
    ...
    def initiate_data_transformation(self) -> DataTransformationArtifact:
        try:
            ...
            preprocessing_obj = self.get_data_transformer_object() # handling missing values
            ...
            train_file_name = os.path.basename(train_file_path).replace(".csv", ".npz") # ziped format
